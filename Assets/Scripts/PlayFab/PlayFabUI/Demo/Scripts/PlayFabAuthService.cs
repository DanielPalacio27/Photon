...
public class PlayFabAuthService  {
    ...
    public static event Action<GetFriendsListResult> OnGetFriendList;
    ...
    private string RememberMeId
    {
        ...
        set
        {
            var guid = string.IsNullOrEmpty(value) ? Guid.NewGuid().ToString() : value; //GUID Universally Unique Identifier, create a unique ID.
    ...
    private void AuthenticateFacebook()
    {
        ...
        if (FB.IsInitialized && FB.IsLoggedIn && !string.IsNullOrEmpty(AuthTicket))
        {
            ...
            }, (result) =>
            {
                ...
                Debug.Log("FACEBOOK LINKED");

            }, (linkError) =>
            {
                //report errro back to the subscriber
                OnPlayFabError?.Invoke(linkError);
            });
    ...
    public void GetFriendList()
    {
        ...
        PlayFabClientAPI.GetFriendsList(request, (friendsList)=> { OnGetFriendList?.Invoke(friendsList); }, (error) => Debug.Log("Loading Friends Error..."));
    }
